with w_months as ( --список обрабатываемых месяцев
  ...
  from   lateral(
              select add_months(trunc(to_date(&end_date, 'yyyymmdd') - 1, 'MM'), -1 * (level - 1)) month_date
              from   dual
...
and    m.month_date between pd.data_nach_vypl and pd.TO_DATE
and    pd.status_pen in ('и', 'п')
and    pd.data_nach_vypl < to_date(&end_date, 'yyyymmdd')
